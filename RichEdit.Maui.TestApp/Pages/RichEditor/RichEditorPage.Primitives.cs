...
public partial class RichEditorPage
{
    ...
    {
        if (_capturedEdit is not { } captured)
            return;
    ...
    {
        if (_capturedEdit is not { Range.IsEmpty: false } captured || _capturedHighlight is null)
            return;
    ...
    {
        if (_trackedRange?.Range is not { } range)
            return;
    ...
    {
        if (_bookmark?.Position is not { } position)
            return;
    ...
    {
        if (_trackingHighlight is null)
            return;

        _trackingHighlight.TrySet(Editor.Document.Revision, _trackedRange?.Range is { IsEmpty: false } range ?
            (RichTextDecoration[])[new(range, new()
                {
                ForegroundColor = StudioTheme.Get("Ink"),
                BackgroundColor = StudioTheme.Get("Selection"),
    ...
    {
        if (PlacementPicker.SelectedItem is not RichTextAdornmentPlacement placement)
            return;
        ...
        if (placement == RichTextAdornmentPlacement.LeftMargin)
            Editor.Adornments.MarginWidth = 96;
    ...
    private void OnCaptureLayout(object? sender, EventArgs args)
    {
        ...
        ResultLabel.Text = _capturedLayout is { } layout ?
            $"Captured layout {layout.LayoutVersion}: {layout.Lines.Count} visible fragments. Caret: {(Editor.TextLayout.GetCaretBounds(
                layout,
                _capturedCaret) is { } caret ? RectText(caret) : "offscreen")}. Edit or scroll, then query this capture." :
    ...
    {
        if (_capturedLayout is not { } layout)
            return;
    ...
    private void OnInspectSelection(object? sender, EventArgs args)
    {
        ...
        if (layout is null)
        {
            ResultLabel.Text = "Native layout is not ready yet.";
            return;
        }
    ...
    private void OnDemoThemeChanged(object? sender, AppThemeChangedEventArgs args)
    {
        PaintTracking();
        UpdateGeometry();
    }
    ...
    private void OnDocumentChanged(object? sender, RichTextDocumentReplacedEventArgs args)
    {
        ClearTracking();
        _capturedLayout = null;
        UpdateState();
    ...
    private void UpdateObservation()
    {
        ...
        if (DemoPicker.SelectedIndex == 4)
            ObservationLabel.Text += $" · {_pointer}";
    }
    ...
    private sealed class GeometryDrawing : IDrawable
    {
        ...
        public void Draw(ICanvas canvas, RectF dirtyRect)
        {
            ...
            foreach (var line in Lines)
                canvas.DrawRectangle((float)line.Bounds.X, (float)line.Bounds.Y, (float)line.Bounds.Width, (float)line.Bounds.Height);

            canvas.FillColor = Colors.Orange.WithAlpha(0.25f);
            foreach (var rect in Selection)
                canvas.FillRectangle((float)rect.X, (float)rect.Y, (float)rect.Width, (float)rect.Height);

            canvas.FillColor = Colors.Crimson;
            if (Caret is { } caret)
                canvas.FillRectangle((float)caret.X, (float)caret.Y, 2, (float)caret.Height);
        }
    }
}
