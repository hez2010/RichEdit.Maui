...
internal sealed class CSharpClassifier
{
        ...
        while (position < text.Length)
        {
            ...
            if (character == '/' && position + 1 < text.Length && text[position + 1] is '/' or '*')
            {
                ...
                while (position < text.Length)
                {
                    ...
                    if (!block && IsNewLine(text[position]))
                        break;
            ...
            else if (char.IsDigit(character))
            {
                ...
                while (position < text.Length)
                {
                    ...
                    if (char.IsLetterOrDigit(next) || next == '_' ||
                        ...
                        !radixLiteral && next is '+' or '-' && text[position - 1] is 'e' or 'E')
                        position++;
                    else
                        break;
                }
            ...
            else if (char.IsLetter(character) || character is '_' or '@')
            {
                ...
                if (Keywords.Contains(text[start..position]))
            ...
            if (kind is { } tokenKind)
                tokens.Add(new CodeToken(new RichTextRange(start, position - start), tokenKind));
        }
    ...
    private static bool TryReadString(string text, ref int position, CancellationToken cancellationToken)
    {
        ...
        if (cursor < text.Length && text[cursor] == '@' && !verbatim)
        {
            verbatim = true;
            cursor++;
        }
        if (cursor >= text.Length || text[cursor] is not ('"' or '\'') ||
            text[cursor] == '\'' && cursor != position)
            return false;
        ...
        else
        {
            ...
            {
                Poll(cursor, cancellationToken);
                if (!verbatim && IsNewLine(text[cursor]))
                    break;

                var next = text[cursor++];
                if (next == quote)
                {
                    if (verbatim && cursor < text.Length && text[cursor] == quote)
                        cursor++;
                    else
                        break;
                }
                else if (!verbatim && next == '\\' && cursor < text.Length && !IsNewLine(text[cursor]))
                    cursor++;
            }
        }
