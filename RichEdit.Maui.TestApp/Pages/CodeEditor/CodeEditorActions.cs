...
internal sealed class CodeEditorActions(CodeEditor editor)
{
    ...
    {
        if (GetCollapseRange() is not { } range)
            return;
    ...
    public void Outdent()
    {
        ...
        for (var line = first; line <= last; line++)
        {
            ...
            var length = 0;
            if (!range.IsEmpty && editor.Document.Text[range.Start] == '\t')
                length = 1;
            else
                while (length < Math.Min(editor.IndentSize, range.Length) && editor.Document.Text[range.Start + length] == ' ')
                    length++;
            if (length > 0)
    ...
    public void ToggleLineComment()
    {
        ...
        for (var line = first; line <= last; line++)
        {
            ...
            while (start < range.End && editor.Document.Text[start] is ' ' or '\t')
                start++;
            if (start != range.End || first == last)
                targets.Add(new RichTextRange(start, range.End - start));
        }
        ...
        foreach (var range in targets)
        {
            if (uncomment)
            {
                ...
                if (length < range.Length && editor.Document.Text[range.Start + length] == ' ')
                    length++;

                edits.Add(new(new RichTextRange(range.Start, length), string.Empty));
            }
            else
                edits.Add(new(new RichTextRange(range.Start, 0), editor.LineCommentPrefix + " "));
        }
    ...
    public RichTextRange? FindNext(string query, CodeSearchOptions options = default, bool wrap = true)
    {
        ...
        foreach (var match in matches)
            if (match.Start >= editor.SelectedRange.End)
    ...
    public RichTextRange? FindPrevious(string query, CodeSearchOptions options = default, bool wrap = true)
    {
        ...
        for (var i = matches.Count - 1; i >= 0; i--)
            if (matches[i].End <= editor.SelectedRange.Start)
                return SelectMatch(matches[i]);
    ...
    private string GetIndentation(int position)
    {
        ...
        var end = range.Start;
        while (end < Math.Min(position, range.End) && editor.Document.Text[end] is ' ' or '\t')
            end++;
    ...
    private (int First, int Last) GetSelectedLines()
    {
        ...
        if (!selection.IsEmpty && last > first && selection.End == editor.GetLineRange(last).Start)
            last--;
    ...
    {
        if (editor.IsReadOnly || edits.Count == 0)
            return false;

        var resultingLength = (long)editor.Document.Length + edits.Sum(static edit => (long)edit.Text.Length - edit.Range.Length);
        if (resultingLength > int.MaxValue || editor.MaxLength >= 0 && resultingLength > editor.MaxLength && resultingLength > editor.Document.Length)
            return false;
