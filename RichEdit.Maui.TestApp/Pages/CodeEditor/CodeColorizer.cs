...
internal sealed partial class CodeColorizer : IDisposable
{
    ...
    internal Func<CodeToken, Color?>? Palette
    {
        ...
        {
            _palette = value;
            if (_revision == _editor.Document.Revision)
                Publish(_revision, Tokens);
        }
    }
    ...
    private async Task RefreshAsync(TimeSpan delay, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await Task.Yield();
            if (token.IsCancellationRequested || _disposed || snapshot.Revision != _editor.Document.Revision)
                return;
            if (Publish(snapshot.Revision, tokens))
            {
                Tokens = tokens;
    ...
    private bool Publish(RichTextRevision revision, IReadOnlyList<CodeToken> tokens)
    {
        ...
        if (_disposed || !ReferenceEquals(Palette, palette))
            return false;
    ...
    private async void Schedule()
    {
        ...
        try
        {
            await RefreshAsync(TimeSpan.FromMilliseconds(120), CancellationToken.None);
        }
