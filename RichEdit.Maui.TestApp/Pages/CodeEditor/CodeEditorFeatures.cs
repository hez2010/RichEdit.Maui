...
internal sealed partial class CodeEditorFeatures : BindableObject, IDisposable
{
    ...
    {
        if (_disposed || _editor.IsReadOnly || _editor.Composition.IsActive)
            return;
        ...
        try
        {
            ...
            if (_disposed || request.IsCancellationRequested || snapshot.Revision != _editor.Document.Revision || selection != _editor.SelectionState || _editor.Composition.IsActive)
                return;
        ...
        {
            if (ReferenceEquals(_completionRequest, request))
    ...
    private void RenderSuggestions()
    {
        Suggestions = _suggestions.Select((text, index) => new CompletionSuggestion(text,
            ...
            new Command(() => AcceptCompletion(index)))
        { IsSelected = index == _suggestion }).ToArray();
    ...
    {
        _suggestion = index;
        for (var item = 0; item < Suggestions.Count; item++)
            Suggestions[item].IsSelected = item == index;
    ...
    {
        if (_disposed || (uint)index >= (uint)_suggestions.Length)
            return false;
    ...
    {
        if (_disposed || _editor.Composition.IsActive)
            return;
    ...
    internal bool NextPlaceholder(bool previous = false)
    {
        ...
        if (_placeholder < 0)
            _placeholder = 0;
        if (_placeholder >= _snippet.Count)
        {
            EndSnippet();
            return true;
        }
    ...
    {
        if (_snippet[_placeholder].Range is not { } range)
        {
            EndSnippet();
            return;
        }
    ...
    {
        if (_disposed || _snippet.Count == 0)
            return;
        if (_snippet.Any(static item => item.Range is null))
        {
            EndSnippet();
            return;
        }
    ...
    private async void OnKeyDown(object? sender, EditorKeyEventArgs args)
    {
        ...
        else if (IsCompletionVisible && args.Modifiers == EditorKeyModifiers.None)
        {
            ...
            else if (args.Key is EditorKey.Enter or EditorKey.Tab)
            {
                args.Handled = true;
                AcceptCompletion(_suggestion);
            }
        }
    ...
    {
        if (_disposed || _editor.Composition.IsActive || args.Buttons != RichTextPointerButtons.None)
            return;

        var layout = _layout.Capture();
        var point = new Point(args.Point.X + _editor.X, args.Point.Y + _editor.Y);
        if (layout is null || _layout.HitTest(layout, point) is not { Kind: RichTextHitKind.Text } hit)
        {
            DismissHover();
            return;
        }
        ...
        if (range == _hoverRange && IsInfoVisible)
            return;
        ...
        try
        {
            ...
            if (_disposed || snapshot.Revision != _editor.Document.Revision || _editor.Composition.IsActive || IsCompletionVisible)
                return;
    ...
    {
        if (_completion is null || _hover is null)
            return;

        var layout = _layout.Capture();
        if (layout is null)
        {
            IsCompletionVisible = IsInfoVisible = false;
            NotifyPopupVisibility();
            return;
        }
        if (IsCompletionVisible && !Place(_completion, _layout.GetCaretBounds(layout, _editor.SelectionState.Active)))
            DismissCompletion();
        if (IsInfoVisible && !Place(_hover, _layout.GetRangeBounds(layout, _hoverRange).FirstOrDefault()))
            DismissHover();

        bool Place(Border popup, Rect? anchor)
        {
            if (anchor is not { Height: > 0 } rect || _host.Width <= 16)
                return false;
    ...
    private async void ScheduleWidgets()
    {
        ...
        try
        {
            ...
            if (_disposed || snapshot.Revision != _editor.Document.Revision || _editor.Composition.IsActive)
                return;
            ...
            if (declaration >= 0 && _actionTemplate is not null)
            {
                ...
                actions.BindingContext = new CodeLineAction("Add Value property", new Command(() =>
                {
                    ...
                    if (brace >= 0)
                        _editor.TryApplyEdits(current.Revision, (RichTextEdit[])[new(new(brace + 1, 0), "\n    public int Value => _value;\n")]);
                }));
            }
        }
        catch (OperationCanceledException) when (request.IsCancellationRequested) { }
        ...
        {
            if (ReferenceEquals(_widgetRequest, request))
                _widgetRequest = null;
        }
    }
    ...
    private void NotifyPopupVisibility()
    {
        OnPropertyChanged(nameof(IsCompletionVisible));
        OnPropertyChanged(nameof(IsInfoVisible));
    }
    ...
    private void OnDocumentChanged(object? sender, CodeDocumentReplacedEventArgs args)
    {
        DismissCompletion();
        DismissHover();
        EndSnippet();
    ...
    private void OnPointerPressed(object? sender, RichTextPointerEventArgs args)
    {
        DismissCompletion();
        DismissHover();
    }
