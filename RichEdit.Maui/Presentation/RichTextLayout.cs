...
public sealed class RichTextLayout
{
    ...
    public event EventHandler? Changed
    {
        add
        {
            ...
            if (_ancestor is not null && _changed is null)
                _editor.TextLayout.Changed += OnSourceLayoutChanged;
    ...
    {
        _editor.VerifyAccess();
        if (Origin is not { } origin || _editor.Handler is not RichEditorHandler handler || handler.CaptureTextLayout() is not { } native)
            return null;
    ...
    public Rect? GetCaretBounds(RichTextLayoutSnapshot layout, int position, RichTextCaretAffinity affinity = RichTextCaretAffinity.Downstream)
    {
        ...
        if (_editor.Folding.FindRange(position) is not null)
            return null;
        if (((RichEditorHandler)_editor.Handler!).GetTextCaretBounds(position, affinity) is not { } caret)
            return null;
    ...
    public IReadOnlyList<Rect> GetRangeBounds(RichTextLayoutSnapshot layout, RichTextRange range)
    {
        ...
        if (range.IsEmpty)
            return GetCaretBounds(layout, range.Start) is { } caret ? (Rect[])[caret] : Array.Empty<Rect>();
        ...
        foreach (var line in layout.Lines)
            foreach (var visible in line.SourceRanges)
            {
                ...
                foreach (var rect in handler.GetTextRangeBounds(new(start, end - start)))
                {
                    ...
                    if (translated.IntersectsWith(layout.TextViewport))
                        rectangles.Add(translated.Intersect(layout.TextViewport));
                }
            }
    ...
    public RichTextHit? HitTest(RichTextLayoutSnapshot layout, Point point)
    {
        ...
        foreach (var adornment in _editor.Adornments.Reverse())
            if (adornment.LayoutBounds.Contains(nativePoint))
                return new(adornment.Position, RichTextCaretAffinity.Downstream, RichTextHitKind.Adornment, adornment);
    ...
    internal void Invalidate()
    {
        ...
        if (!_editor.Dispatcher.Dispatch(() =>
        {
    ...
    internal Point? Origin
    {
        get
        {
            if (_ancestor is null || ReferenceEquals(_ancestor, _editor))
                return Point.Zero;

            var point = Point.Zero;
            for (Element? view = _editor; view is not null; view = view.Parent)
            {
                if (ReferenceEquals(view, _ancestor))
                    return point;
                if (view is VisualElement element)
                    point = new(point.X + element.X + element.TranslationX, point.Y + element.Y + element.TranslationY);
            }
