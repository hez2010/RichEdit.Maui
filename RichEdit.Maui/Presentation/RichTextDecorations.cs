...
public sealed class RichTextDecorations
{
    ...
    internal void Set(RichTextDecorationLayer layer, ImmutableArray<RichTextDecoration> items)
    {
        ...
        if ((_editor.Handler as IRichEditorHandler)?.IsComposing == true)
        {
            if (layer.PendingItems.IsDefault)
                layer.PaintedItems = layer.Items;
        ...
        if (!hadPending && _dirtyRange is null && layer.Items.AsSpan().SequenceEqual(items.AsSpan()))
            return;
    ...
    {
        _editor.VerifyAccess();
        if (revision != _editor.Document.Revision)
            return false;
    ...
    {
        if (painted && _layers.All(static layer => layer.PendingItems.IsDefault))
            painted = false;
        if (ReferenceEquals(snapshot, _projection))
            return snapshot;
        if (!painted && ReferenceEquals(snapshot, _projectionSource))
            return _projection!;
        ...
        foreach (var layer in _layers)
        {
            ...
            foreach (var run in projected.Runs)
            {
                ...
                {
                    while (index < items.Length && items[index].Range.End <= position)
                        index++;
                    ...
                    if (index < items.Length)
                    {
                        ...
                        if (decoration.Range.Start <= position)
                        {
                            ...
                            end = Math.Min(end, decoration.Range.Start);
                    }
    ...
    internal RichTextDocumentSnapshot RestoreAuthoredSnapshot(RichTextDocumentSnapshot observed, RichTextRange replacement)
    {
        ...
        if (ReferenceEquals(source, projected))
            return observed;
        ...
        foreach (var run in observed.Runs)
        {
            for (var position = run.Start; position < run.End;)
            {
                while (authored.Runs[authoredIndex].End <= position)
                    authoredIndex++;

                while (projected.Runs[projectedIndex].End <= position)
                    projectedIndex++;
    ...
    private static RichTextCharacterFormat Restore(RichTextCharacterFormat observed, RichTextCharacterFormat authored, RichTextCharacterFormat display) => observed with
        {
            ...
            ForegroundColor = !Equals(
                authored.ForegroundColor,
                display.ForegroundColor) && SameColor(
                    observed.ForegroundColor,
                    display.ForegroundColor) ? authored.ForegroundColor : observed.ForegroundColor,
            BackgroundColor = !Equals(
                authored.BackgroundColor,
                display.BackgroundColor) && SameColor(
                    observed.BackgroundColor,
                    display.BackgroundColor) ? authored.BackgroundColor : observed.BackgroundColor,
    ...
    {
        if (!changes.IsTextChanged || _layers.All(static layer => layer.Items.IsEmpty && layer.PendingItems.IsDefaultOrEmpty))
            return;
    ...
    private void QueueRefresh()
    {
        ...
        if (!_editor.Dispatcher.DispatchDelayed(TimeSpan.FromMilliseconds(40), () =>
        {
            ...
            foreach (var layer in _layers.ToArray())
            {
                ...
                {
                    if (layer.PendingRevision != _editor.Document.Revision)
                        layer.Items = layer.PaintedItems;
