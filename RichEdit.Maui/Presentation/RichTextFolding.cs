...
public sealed class RichTextFolding
{
    ...
    public void SetCollapsedRanges(IEnumerable<RichTextRange> ranges)
    {
        ...
        foreach (var range in items)
        {
            ...
            if (range.IsEmpty)
                throw new ArgumentException("Collapsed ranges must be nonempty.", nameof(ranges));
        }
        if (_ranges.AsSpan().SequenceEqual(items.AsSpan()))
            return;
    ...
    {
        if (_ranges.IsEmpty || !changes.IsTextChanged)
            return false;
    ...
    {
        if (_ranges.AsSpan().SequenceEqual(ranges.AsSpan()))
            return;
