...
public partial class RichEditorHandler
{
    ...
    private void CustomizeTextFlyout(TextCommandBarFlyout flyout)
    {
        ...
        if (menu.IncludeDefaultItems && menu.Items.Count == 0)
            return;
        ...
        try
        {
            ...
            foreach (var item in menu.Items)
            {
                ...
                if (item is IMenuFlyoutSubItem submenu)
                    button.Flyout = CreateMenuFlyout(submenu, flyout.Hide);
                else
                    button.Click += (_, _) =>
                    {
    ...
    private static NativeMenuFlyout CreateMenuFlyout(IEnumerable<IMenuElement> items, Action close)
    {
        ...
        foreach (var item in items)
            flyout.Items.Add(CreateMenuItem(item, close));
    ...
    {
        if (sender is TextCommandBarFlyout flyout)
            RestoreTextFlyout(flyout);
    }

    private void RestoreTextFlyout(TextCommandBarFlyout flyout)
    {
        if (!_textFlyoutContents.Remove(flyout, out var contents))
            return;

        flyout.PrimaryCommands.Clear();
        flyout.SecondaryCommands.Clear();
        foreach (var item in contents.Primary)
            flyout.PrimaryCommands.Add(item);

        foreach (var item in contents.Secondary)
