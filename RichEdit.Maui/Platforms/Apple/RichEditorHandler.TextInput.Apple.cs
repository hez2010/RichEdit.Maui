...
namespace RichEdit.Maui.Platforms.Apple
{
    public partial class RichTextView
    {
        ...
        private readonly struct DisplayInputScope : IDisposable
        {
            ...
            internal DisplayInputScope(RichTextView view)
            {
                _view = view;
                view._displayInputDepth++;
            }
        ...
        public override string TextInRange(UITextRange range)
        {
            ...
            string? text;
            using (new DisplayInputScope(this))
                text = base.TextInRange(range);
        ...
        {
            if (SourceInputProjection is not { } map || offset == 0)
                return base.GetPosition(from, offset);

            var position = (long)map.ToSource((int)GetDisplayOffset(BeginningOfDocument, from)) + offset;
            if (position < 0 || position > map.ToSource((int)TextStorage.Length))
                return null!;
        ...
        {
            if (SourceInputProjection is null)
                return base.GetPosition(range, offset);
                ...
                do
                {
                    ...
                    if (next is null || GetDisplayOffset(current, next) == 0)
                    {
                        if (!ReferenceEquals(current, from))
                            current.Dispose();
                        if (next is not null && !ReferenceEquals(next, current) && !ReferenceEquals(next, from))
                            next.Dispose();
        ...
        {
            if (SourceInputProjection is not { } map)
                return position;
        ...
        public override UITextPosition GetClosestPositionToPoint(CGPoint point, UITextRange range)
        {
            ...
            using (new DisplayInputScope(this))
                position = base.GetClosestPositionToPoint(point, range);
        ...
        public override UITextPosition GetPositionWithinRange(UITextRange range, UITextLayoutDirection direction)
        {
            UITextPosition position;
            using (new DisplayInputScope(this))
                position = base.GetPositionWithinRange(range, direction);
            return NormalizeSourcePosition(position);
        }
        ...
        {
            if (SourceInputProjection is null)
                return base.GetCharacterRange(position, direction);
