...
namespace RichEdit.Maui
{
    public partial class RichEditorHandler
    {
        internal NSAttributedString SourceAccessibilityText()
        {
            ...
            foreach (var item in map.Reservations.Reverse())
                value.DeleteRange(new(item.DisplayStart, item.Text.Length));
...
namespace RichEdit.Maui.Platforms.Apple
{
    public partial class RichTextView : IUIAccessibilityReadingContent
    {
        ...
        {
            if (ProjectionHandler is not { VirtualView: var editor } || editor.TextLayout.Capture() is not { } layout)
                return -1;

            var local = ConvertPointFromView(point, null);
            if (editor.TextLayout.HitTest(layout, new(local.X, local.Y)) is not { } hit)
                return -1;
        ...
        {
            if (ProjectionHandler is not { VirtualView.Document.CurrentSnapshot: var source } || line < 0 || line >= source.Paragraphs.Length)
                return null;
        ...
        {
            if (AccessibilityLine(lineNumber) is not { } line || ProjectionHandler!.VirtualView.TextLayout.Capture() is not { } layout)
                return CGRect.Empty;
