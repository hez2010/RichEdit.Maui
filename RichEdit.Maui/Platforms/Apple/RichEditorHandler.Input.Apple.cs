...
namespace RichEdit.Maui.Platforms.Apple
{
    public partial class RichTextView
    {
        ...
        internal void ResetKeyInput()
        {
            ...
            _handledKeys.Clear();
            foreach (var command in _bindingCommands)
                command.Dispose();
        ...
        public override UIKeyCommand[] KeyCommands
        {
            get
            {
                if (MarkedTextRange is not null || EditorKeyBindings is not { } bindings)
                ...
                if (_keyBindingVersion != bindings.Version)
                {
                    ...
                    for (var index = bindings.Count - 1; index >= 0; index--)
                    {
                        ...
                        if (!gestures.Add((binding.Key, binding.Modifiers)) ||
                            (binding.Modifiers & EditorKeyModifiers.AltGraph) != 0 || GetKeyCommandInput(binding.Key) is not { } input)
                            continue;

                        UIKeyModifierFlags flags = 0;
                        if ((binding.Modifiers & EditorKeyModifiers.Shift) != 0)
                            flags |= UIKeyModifierFlags.Shift;
                        if ((binding.Modifiers & EditorKeyModifiers.Control) != 0)
                            flags |= UIKeyModifierFlags.Control;
                        if ((binding.Modifiers & EditorKeyModifiers.Alt) != 0)
                            flags |= UIKeyModifierFlags.Alternate;
                        if ((binding.Modifiers & EditorKeyModifiers.Meta) != 0)
                            flags |= UIKeyModifierFlags.Command;
                        if (binding.Key is >= EditorKey.NumPad0 and <= EditorKey.Divide)
        ...
        {
            if (action.Handle != KeyBindingAction.Handle)
        ...
        {
            if (MarkedTextRange is not null || command.PropertyList is not NSNumber gesture)
                return;
        ...
        {
            if (key is >= EditorKey.A and <= EditorKey.Z)
                return ((char)('a' + key - EditorKey.A)).ToString();
            if (key is >= EditorKey.D0 and <= EditorKey.D9)
                return ((char)('0' + key - EditorKey.D0)).ToString();
            if (key is >= EditorKey.NumPad0 and <= EditorKey.NumPad9)
                return ((char)('0' + key - EditorKey.NumPad0)).ToString();
        ...
        private bool HandleKeyDown(UIPress press)
        {
            ...
            if (_handledKeys.Contains(code))
                return true;
            if (MarkedTextRange is not null || KeyDownRequested is not { } requested)
                return false;

            var modifiers = EditorKeyModifiers.None;
            if ((key.ModifierFlags & UIKeyModifierFlags.Shift) != 0)
                modifiers |= EditorKeyModifiers.Shift;
            if ((key.ModifierFlags & UIKeyModifierFlags.Control) != 0)
                modifiers |= EditorKeyModifiers.Control;
            if ((key.ModifierFlags & UIKeyModifierFlags.Alternate) != 0)
                modifiers |= EditorKeyModifiers.Alt;
            if ((key.ModifierFlags & UIKeyModifierFlags.Command) != 0)
            ...
            if (!requested(GetEditorKey(key), modifiers))
            {
                ...
                if (MoveSourceVertically(GetEditorKey(key), modifiers))
                {
        ...
        private static void ForwardPresses(NSSet<UIPress> presses, Func<UIPress, bool> include, Action<NSSet<UIPress>> forward)
        {
            ...
            if (remaining.Length == (int)presses.Count)
                forward(presses);
        ...
        private static EditorKey GetEditorKey(UIKey key)
        {
            ...
            if (code is >= 30 and <= 38)
                return EditorKey.D1 + (code - 30);
            if (code is >= 58 and <= 69)
                return EditorKey.F1 + (code - 58);
            if (code is >= 104 and <= 115)
                return EditorKey.F13 + (code - 104);
            if (code is >= 89 and <= 97)
                return EditorKey.NumPad1 + (code - 89);
    ...
    public partial class RichEditorHandler
    {
        ...
        private UIMenu CreateAppleContextMenu(UIMenuElement[] suggestedActions)
        {
            ...
            if (menu.IncludeDefaultItems)
                elements.AddRange(suggestedActions);
