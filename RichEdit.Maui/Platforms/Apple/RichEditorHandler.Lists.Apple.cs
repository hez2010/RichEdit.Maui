    ...
    public partial class RichEditorHandler
    {
        ...
        private static NativeListMetadata? GetNativeListMetadata(NSParagraphStyle style)
        {
            if ((!OperatingSystem.IsIOSVersionAtLeast(16) && !OperatingSystem.IsMacCatalystVersionAtLeast(16)) ||
                style.TextLists is not { Length: > 0 } lists)
                return null;
        ...
        {
            if (format.NativeList is null)
                return format.LeadingIndent + format.FirstLineIndent;
        ...
        internal void DrawListMarkers(CGRect rect)
        {
            ...
            {
                if (paragraph.Start > visibleCharacters.Location + visibleCharacters.Length)
                    break;
                ...
                if (paragraph.Start < visibleCharacters.Location || FindDisplayFoldRange(paragraph.Start) is not null ||
                    paragraph.Start > PlatformView.TextStorage.Length)
                    continue;
                ...
                if (caret.Bottom < rect.Top)
                    continue;
                if (caret.Top > rect.Bottom)
                    break;
