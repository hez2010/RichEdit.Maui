...
namespace RichEdit.Maui
{
    public partial class RichEditorHandler
    {
        ...
        private partial void ConnectInputObservation()
        {
            ...
            {
                if (recognizer.State is UIGestureRecognizerState.Ended or UIGestureRecognizerState.Cancelled)
                {
        ...
        {
            if (_nativeEditGeneration != _readNativeGeneration || PlatformView.TextStorage.Value != DisplaySourceSnapshot.Text)
                OnNativeDocumentChanged(PlatformView);
    ...
    public partial class RichTextView
    {
        ...
        internal bool MoveSourceVertically(EditorKey key, EditorKeyModifiers modifiers)
        {
            if (ProjectionHandler is not { NativeProjection.IsEmpty: false } handler || key is not (EditorKey.Up or EditorKey.Down) ||
                modifiers is not (EditorKeyModifiers.None or EditorKeyModifiers.Shift))
                return false;
