...
namespace RichEdit.Maui
{
    public partial class RichEditorHandler
    {
        ...
        {
            if (PlatformView.EditableText is not { } text)
                return default;
        ...
        private void OnObservedPointer(MotionEvent e)
        {
            ...
            {
                VirtualView.ObservePointerExit();
                return;
            }
            if (e.ActionMasked is not (MotionEventActions.Down or MotionEventActions.Move or MotionEventActions.HoverMove or MotionEventActions.HoverEnter))
                return;
            ...
            if ((e.ButtonState & MotionEventButtonState.Primary) != 0 || kind == RichTextPointerDeviceKind.Touch)
                buttons |= RichTextPointerButtons.Primary;
            if ((e.ButtonState & MotionEventButtonState.Secondary) != 0)
                buttons |= RichTextPointerButtons.Secondary;
            if ((e.ButtonState & MotionEventButtonState.Tertiary) != 0)
                buttons |= RichTextPointerButtons.Middle;

            var modifiers = EditorKeyModifiers.None;
            if ((e.MetaState & MetaKeyStates.ShiftOn) != 0)
                modifiers |= EditorKeyModifiers.Shift;
            if ((e.MetaState & MetaKeyStates.CtrlOn) != 0)
                modifiers |= EditorKeyModifiers.Control;
            if ((e.MetaState & MetaKeyStates.AltOn) != 0)
                modifiers |= EditorKeyModifiers.Alt;
            if ((e.MetaState & MetaKeyStates.MetaOn) != 0)
    ...
    public partial class RichEditText
    {
        ...
        public override IInputConnection? OnCreateInputConnection(EditorInfo? outAttrs)
        {
            ...
            if (outAttrs is not null && ProjectionHandler is { } handler && !handler.NativeProjection.IsEmpty)
            {
                ...
                if (OperatingSystem.IsAndroidVersionAtLeast(30))
                    outAttrs.SetInitialSurroundingText(handler.VirtualView.Document.Text);
            }
        ...
        private sealed partial class ObservedInputConnection(RichEditText owner, IInputConnection target) : InputConnectionWrapper(target, false)
        {
            private bool Observe(bool starts, Func<bool> operation)
            {
                ...
                {
                    owner.CompositionCompleted?.Invoke();
                }
            }
