...
public partial class RichEditorHandler
{
    ...
    internal IReadOnlyList<Rect> GetTextRangeBounds(RichTextRange range)
    {
        ...
        if (NativeProjection.IsEmpty)
            return GetTextRangeBoundsCore(range);
        ...
        foreach (var item in NativeProjection.Reservations)
        {
            if (item.SourcePosition <= start)
                continue;
            if (item.SourcePosition >= range.End)
                break;
        ...
        if (start < range.End)
            result.AddRange(GetTextRangeBoundsCore(NativeProjection.ToDisplay(new RichTextRange(start, range.End - start))));
    ...
    {
        if (start < end && NativeProjection.ToSource(start) == NativeProjection.ToSource(end))
            return [];
        ...
        if (start == end && VirtualView.Folding.FindRange(start) is null)
            result.Add(new(start, 0));
        ...
        foreach (var hidden in VirtualView.Folding.EffectiveRanges)
        {
            ...
            if (hidden.Start >= end)
                break;
            if (hidden.Start > start)
                result.Add(new(start, hidden.Start - start));
    ...
    private RichTextHit SourceHit(int position, RichTextCaretAffinity affinity, bool isText)
    {
        ...
        if (VirtualView.Folding.FindRange(position) is { } hidden)
            position = hidden.End;
