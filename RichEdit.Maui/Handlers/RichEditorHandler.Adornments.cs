...
public partial class RichEditorHandler
{
    ...
    private void DisconnectAdornments()
    {
        ...
        foreach (var adornment in VirtualView.Adornments)
            adornment.View.Handler?.DisconnectHandler();
    }

    internal void UpdateAdornments()
    {
        if (!_adornmentsConnected || _arrangingAdornments)
            return;
    ...
    private void QueueAdornmentLayout()
    {
        if (_adornmentsConnected)
            VirtualView.TextLayout.Invalidate();
        if (!_adornmentsConnected || _adornmentLayoutQueued || VirtualView.Adornments.Count == 0 && NativeProjection.IsEmpty)
            return;
    ...
    private void ArrangeAdornments()
    {
        ...
        try
        {
            ...
            foreach (var item in adornments)
            {
                ...
                {
                    if (realized++ >= AdornmentBatchSize)
                    {
                        pending = true;
                        continue;
                    }
                ...
                else if (item.IsRealized && !nearby && !HasFocusedContent(item.View))
                    adornments.Overlay.SetRealized(item, false);
                if (item.IsRealized && item.MeasureDirty)
                    _displayAdornmentVersion = -1;
            }
            ...
            foreach (var item in adornments)
            {
                ...
                if (item.ReservesSpace)
                {
                    if (reservations.TryGetValue(item, out var reservation) &&
                        GetDisplayReservationBounds(reservation) is { } reserved && reserved.IntersectsWith(viewport))
                        bounds = reserved;
                }
                else if (VirtualView.Folding.FindRange(item.Position) is null && GetAdornmentAnchor(NativeProjection.ToDisplay(item.Position)) is { Height: > 0 } anchor)
                {
                    ...
                    if (proposed.IntersectsWith(viewport))
                        bounds = proposed;
                }
