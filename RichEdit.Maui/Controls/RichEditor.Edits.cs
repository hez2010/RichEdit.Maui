...
public partial class RichEditor
{
        ...
        RichTextSelectionState? selectionAfter = null, RichTextEditOptions options = default)
    {
        ...
        if (!Enum.IsDefined(options.UndoBehavior))
        ...
        {
            if (group.Key < previousEnd)
                throw new ArgumentException("Edit ranges overlap.", nameof(edits));
        ...
        if (length > int.MaxValue || MaxLength >= 0 && length > MaxLength && length > source.Length)
            return false;
