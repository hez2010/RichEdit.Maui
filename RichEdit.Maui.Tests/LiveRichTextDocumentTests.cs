...
{
    private static readonly RichTextListDefinition BulletList = new(
    [
    ...
    [Fact]
    public void NativeEnterInNonemptyListItemContinuesTheList()
    {
        var document = new RichTextDocument();
        RichTextListId listId = default;
        document.Edit(edit =>
        {
            edit.InsertText(0, "item");
            listId = edit.CreateList(BulletList);
            edit.ApplyList(new RichTextRange(0, 4), listId);
        });

        var result = document.CurrentSnapshot.Replace(
            4..4,
            "\n",
            replacementFormat: null,
            out var affectedParagraphs);

        Assert.Equal("item\n", result.Text);
        Assert.Equal(new RichTextRange(0, 5), affectedParagraphs);
        Assert.All(result.Paragraphs, paragraph =>
            Assert.Equal(listId, paragraph.Format.List?.ListId));
    }

    [Fact]
    public void NativeEnterInEmptyListItemInsertsTwoNonlistParagraphs()
    {
        var document = new RichTextDocument();
        RichTextListId listId = default;
        document.Edit(edit =>
        {
            ...
            listId = edit.CreateList(BulletList);
            edit.ApplyList(new RichTextRange(0, 4), listId);
            edit.ApplyList(new RichTextRange(5, 0), listId);
        });
        ...
        Assert.Equal("item\n\n", result.Text);
        Assert.Equal(new RichTextRange(5, 1), affectedParagraphs);
        Assert.Equal(listId, result.GetParagraphFormat(0).List?.ListId);
        Assert.Null(result.GetParagraphFormat(5).List);
        Assert.Null(result.GetParagraphFormat(6).List);
    }

    [Fact]
    public void NativeEnterInNestedEmptyListItemExitsTheListRatherThanOutdenting()
    {
        var document = new RichTextDocument();
        RichTextListId listId = default;
        document.Edit(edit =>
        {
            ...
            listId = edit.CreateList(NumberedOutline);
            edit.ApplyList(new RichTextRange(0, 4), listId);
            edit.ApplyList(new RichTextRange(5, 0), listId, level: 1);
        });

        var result = document.CurrentSnapshot.Replace(5..5, "\n");

        Assert.Equal(0, result.GetParagraphFormat(0).List?.Level);
        Assert.Null(result.GetParagraphFormat(5).List);
        Assert.Null(result.GetParagraphFormat(6).List);
    }

    [Fact]
    public void NativeDeleteOfDelimiterAfterNonemptyParagraphKeepsLeftListFormat()
    {
        var document = new RichTextDocument();
        RichTextListId leftList = default;
        RichTextListId rightList = default;
        document.Edit(edit =>
        {
            edit.InsertText(0, "left\nright");
            leftList = edit.CreateList(BulletList);
            rightList = edit.CreateList(NumberedOutline);
            edit.ApplyList(new RichTextRange(0, 4), leftList);
            edit.ApplyList(new RichTextRange(5, 5), rightList);
        });

        var result = document.CurrentSnapshot.Replace(4..5, string.Empty);

        Assert.Equal("leftright", result.Text);
        Assert.Equal(leftList, result.GetParagraphFormat(0).List?.ListId);
    }

    [Fact]
    public void NativeDeleteOfEmptyParagraphDelimiterUsesRightListFormat()
    {
        var document = new RichTextDocument();
        RichTextListId emptyList = default;
        RichTextListId rightList = default;
        document.Edit(edit =>
        {
            edit.InsertText(0, "\nright");
            emptyList = edit.CreateList(BulletList);
            rightList = edit.CreateList(NumberedOutline);
            edit.ApplyList(new RichTextRange(0, 0), emptyList);
            edit.ApplyList(new RichTextRange(1, 5), rightList);
        });

        var result = document.CurrentSnapshot.Replace(0..1, string.Empty);

        Assert.Equal("right", result.Text);
        Assert.Equal(rightList, result.GetParagraphFormat(0).List?.ListId);
    }

    [Fact]
    public void NativeSelectionDeletionAcrossParagraphsUsesEndingParagraphFormat()
    {
        var document = new RichTextDocument();
        document.Edit(edit =>
        {
            edit.InsertText(0, "left\nright");
            var listId = edit.CreateList(BulletList);
            edit.ApplyList(new RichTextRange(0, 4), listId);
        });

        var result = document.CurrentSnapshot.Replace(2..7, string.Empty);

        Assert.Equal("leght", result.Text);
        Assert.Null(result.GetParagraphFormat(0).List);
    }

    [Fact]
    public void NativeSelectionDeletionFromNonlistIntoListUsesEndingListFormat()
    {
        var document = new RichTextDocument();
        RichTextListId listId = default;
        document.Edit(edit =>
        {
            edit.InsertText(0, "left\nright");
            listId = edit.CreateList(BulletList);
            edit.ApplyList(new RichTextRange(5, 5), listId);
        });
    ...
    [Fact]
    public void NativeEnterInMiddleEmptyListItemDoesNotChangeFollowingParagraph()
    {
        var document = new RichTextDocument();
        RichTextListId listId = default;
        document.Edit(edit =>
        ...
        Assert.Equal("item\n\n\nfollowing", result.Text);
        Assert.Equal(listId, result.GetParagraphFormat(0).List?.ListId);
        Assert.Null(result.GetParagraphFormat(5).List);
        Assert.Null(result.GetParagraphFormat(6).List);
        Assert.Null(result.GetParagraphFormat(7).List);
    }

    [Fact]
    public void NativeDeletionInsideFollowingTextDoesNotChangePrecedingEmptyListItem()
    {
        var document = new RichTextDocument();
        RichTextListId listId = default;
        document.Edit(edit =>
        ...
        var result = document.CurrentSnapshot.Replace(7..10, string.Empty);

        Assert.Equal(listId, result.GetParagraphFormat(5).List?.ListId);
        Assert.Null(result.GetParagraphFormat(6).List);
    }

    [Fact]
    public void NativeDeletionOfFollowingLineAfterEmptyListLeavesNonlistEmptyParagraph()
    {
        var document = new RichTextDocument();
        document.Edit(edit =>
        {
        ...
        var result = document.CurrentSnapshot.Replace(5..document.Length, string.Empty);

        Assert.Equal("item\n", result.Text);
        Assert.NotNull(result.GetParagraphFormat(0).List);
        Assert.Null(result.GetParagraphFormat(result.Length).List);
    }

    [Fact]
    public void NativeSoftReturnDoesNotSplitOrTerminateAListParagraph()
    {
        var document = new RichTextDocument();
        RichTextListId listId = default;
        document.Edit(edit =>
        ...
        var result = document.CurrentSnapshot.Replace(
            4..4,
            RichTextDocument.SoftLineBreakCharacter.ToString(),
            replacementFormat: null,
            out var affectedParagraphs);

        Assert.Equal($"item{RichTextDocument.SoftLineBreakCharacter}", result.Text);
        Assert.Null(affectedParagraphs);
        Assert.Single(result.Paragraphs);
        Assert.Equal(listId, result.GetParagraphFormat(0).List?.ListId);
    }

    [Fact]
    public void FirstBackspaceCommandAtListStartRemovesOnlyThatItemsListState()
    {
        var document = new RichTextDocument();
        RichTextListId listId = default;
        document.Edit(edit =>
        ...
        document.Edit(edit => edit.RemoveList(new RichTextRange(6, 0)));

        Assert.Equal(text, document.Text);
        Assert.Equal(listId, document.CurrentSnapshot.GetParagraphFormat(0).List?.ListId);
        Assert.Null(document.CurrentSnapshot.GetParagraphFormat(6).List);
    }
