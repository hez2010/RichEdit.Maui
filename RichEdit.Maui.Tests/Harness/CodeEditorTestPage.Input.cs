...
internal sealed partial class CodeEditorTestPage
{
    private async Task RunInputTests(Func<string, string, Func<Task>, Task> test)
    {
        ...
        async Task Test(string name, Func<Task> action) => await test(name, "text", async () =>
        {
            ...
            finally
            {
                ...
                foreach (var binding in defaults)
                    _editor.KeyBindings.Add(binding);
        ...
        await Test("last binding owns gesture and rechecks command parameter", () =>
        {
            ...
            _textView.KeyBindings.Add(new(EditorKey.S, EditorKeyModifiers.Control,
                ...
                {
                    Equal(true, ReferenceEquals(parameter, value));
                    invoked++;
                },
        ...
        await Test("key binding collection rejects background mutation", async () =>
        {
            ...
            var rejected = await Task.Run(() =>
            {
                try
                {
                    _editor.KeyBindings.Add(new(EditorKey.S, EditorKeyModifiers.Control, new Command(static () => { })));
                }
        ...
        await Test("Android command exceptions reach native key callback caller", () =>
        {
            ...
            try
            {
                native.OnKeyDown(Android.Views.Keycode.F1, key);
            }
    ...
    private void SendNativeKey(EditorKey key, EditorKeyModifiers modifiers = EditorKeyModifiers.None)
    {
        ...
        if ((modifiers & EditorKeyModifiers.Control) != 0)
            flags |= Android.Views.MetaKeyStates.CtrlOn;
        if ((modifiers & EditorKeyModifiers.Alt) != 0)
            flags |= Android.Views.MetaKeyStates.AltOn;
        if ((modifiers & EditorKeyModifiers.Shift) != 0)
            flags |= Android.Views.MetaKeyStates.ShiftOn;
        if ((modifiers & EditorKeyModifiers.Meta) != 0)
            flags |= Android.Views.MetaKeyStates.MetaOn;
        if ((modifiers & EditorKeyModifiers.AltGraph) != 0)
            flags |= Android.Views.MetaKeyStates.AltRightOn;
        ...
        if ((modifiers & EditorKeyModifiers.Control) != 0)
            flags |= UIKit.UIKeyModifierFlags.Control;
        if ((modifiers & EditorKeyModifiers.Alt) != 0)
            flags |= UIKit.UIKeyModifierFlags.Alternate;
        if ((modifiers & EditorKeyModifiers.Shift) != 0)
            flags |= UIKit.UIKeyModifierFlags.Shift;
        if ((modifiers & EditorKeyModifiers.Meta) != 0)
            flags |= UIKit.UIKeyModifierFlags.Command;
    ...
    private sealed class NativeInputMenu : IDisposable
    {
        ...
        public void Dispose()
        {
            _callback.OnDestroyActionMode(null);
            _popup.Dispose();
        }
#else
        ...
        private void Read(UIKit.UIMenu menu)
        {
            foreach (var item in menu.Children)
            {
                if (item.Title.Length != 0)
                    _items.Add(item);
                if (item is UIKit.UIMenu submenu)
                    Read(submenu);
            }
        }
