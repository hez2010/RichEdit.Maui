...
internal static partial class EditorContractTests
{
    ...
    {
        foreach (var test in FoldingCases())
            yield return test;

        foreach (var test in AdornmentCases())
            yield return test;

        foreach (var test in LayoutCases())
            yield return test;

        foreach (var test in ProjectionCases())
            yield return test;

        foreach (var test in PerformanceCases())
            yield return test;
#if ANDROID || IOS || MACCATALYST
        foreach (var test in CharacterFormattingTests.Cases)
            yield return test;
#endif
        ...
        yield return new("background mutation fails before native or document changes", async editor =>
        {
            ...
            var failure = await Task.Run(() =>
            {
                try
                {
                    editor.Document.Edit(edit => edit.DeleteText(new RichTextRange(0, 3)));
                    return null;
                }
        ...
        yield return new("native rendering attributes match authored character and paragraph formats", async editor =>
        {
            ...
            Equal(
                true,
                attributes.Font!.FontDescriptor.SymbolicTraits.HasFlag(UIKit.UIFontDescriptorSymbolicTraits.Bold),
                $"font={attributes.Font}; model={editor.Document.CurrentSnapshot.Runs[0].Format}");
                ...
                void Change(object? sender, RichTextPastingEventArgs args)
                {
                    switch (mutation)
                    {
                        case "selection":
                            ...
                            break;
                        case "document":
                            editor.Document = RichTextDocument.FromRtf(@"{\rtf1 replacement}");
                            break;
                        case "content":
                            editor.Document.Edit(edit => edit.InsertText(0, "changed "));
                            break;
                        case "read-only":
                            editor.IsReadOnly = true;
                            break;
        ...
        foreach (var seed in new[] { 8149, 20260906, 42 })
        {
            yield return new($"mixed native and programmatic transaction replay seed {seed}", async editor =>
            {
                ...
                var count = 0;
                while (editor.CanUndo && count++ < 250)
                {
                    editor.Undo();
                    await Verify(editor);
                }
                ...
                Equal(false, editor.CanUndo);
                count = 0;
                while (editor.CanRedo && count++ < 250)
                {
                    editor.Redo();
                    await Verify(editor);
                }
    ...
    internal static IEnumerable<(string, RichTextCharacterFormat)> CharacterFormats()
    {
        ...
        foreach (var style in Enum.GetValues<RichTextUnderlineStyle>())
            yield return ($"underline {style}", f with { Underline = style });
    ...
    private static IEnumerable<(string, RichTextParagraphFormat)> ParagraphFormats()
    {
        ...
        foreach (var alignment in Enum.GetValues<RichTextAlignment>())
            yield return ($"alignment {alignment}", f with { Alignment = alignment });

        foreach (var rule in Enum.GetValues<RichTextLineSpacingRule>())
            yield return ($"line spacing {rule}", f with
            {
                LineSpacingRule = rule,
                LineSpacing = rule == RichTextLineSpacingRule.Multiple ? 1.5 : rule is RichTextLineSpacingRule.AtLeast or RichTextLineSpacingRule.Exactly ? 24 : 0
            });
        ...
        yield return ("shading", f with { Shading = 2500, ShadingForegroundColor = Colors.Red, ShadingBackgroundColor = Colors.Yellow });
        yield return ("borders", f with
        {
            Border = new RichTextBorder(RichTextBorderSides.Top | RichTextBorderSides.Bottom, RichTextBorderStyle.Dashed, 1.5, Colors.Blue)
        });
        yield return ("style name", f with { StyleName = "Custom" });
    }
    ...
    private static RichTextListDefinition List(RichTextListNumberStyle style) => new((RichTextListLevelDefinition[])[
        new RichTextListLevelDefinition
            {
            Marker = new RichTextListMarker.Number(style, 3),
            Prefix = "(",
            Suffix = ")",
            LeadingIndent = 24,
            FirstLineIndent = -12,
        ...
        new RichTextListLevelDefinition
            {
            Marker = new RichTextListMarker.Number(style, 1),
            Prefix = "",
            Suffix = ".",
            LeadingIndent = 48,
            FirstLineIndent = -12,
    ...
    public static async Task Verify(RichEditor editor)
    {
        ...
        for (var attempt = 0; attempt < 50; attempt++)
        {
            ...
            if (native == editor.Document.Text)
                break;
        }
    ...
    {
        if (text.Length == 0 && editor.SelectedRange.IsEmpty)
            return;
#if WINDOWS
