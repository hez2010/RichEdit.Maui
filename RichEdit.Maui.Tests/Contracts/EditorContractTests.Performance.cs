    ...
    private static IEnumerable<Case> PerformanceCases()
    {
        yield return new("performance dense presentation on 2000 source lines", async editor =>
        {
            ...
            for (var pass = 0; pass < 400; pass++)
            {
                ...
                {
                    longestDelay = Math.Max(longestDelay, queue.Elapsed.TotalMilliseconds);
                    ready.SetResult();
                });
                await ready.Task;
                await Task.Delay(20);
                if (pass % 10 == 0)
                    Progress($"batch checkpoint {pass}: source {editor.Document.Length}, native {NativeText(editor).Length}");
                if (NativeText(editor).Length == source.Length + widgets.Count)
                    break;
            }
