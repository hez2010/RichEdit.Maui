...
public sealed partial class CodeEditor : ContentView
{
    ...
    public static readonly BindableProperty DocumentProperty = BindableProperty.Create(
        ...
        coerceValue: static (view, value) =>
        {
            ...
            try
            {
                ...
                if (ReferenceEquals(editor.TextView.Document, requested.Source))
                    return requested;
    ...
    public static readonly BindableProperty SelectionStateProperty = BindableProperty.Create(
        ...
        {
            var editor = (CodeEditor)view;
            if (!editor._synchronizingSelection)
                editor.TextView.SelectionState = (RichTextSelectionState)value;
        });
    ...
    public static readonly BindableProperty SelectedRangeProperty = BindableProperty.Create(
        ...
        propertyChanged: static (view, _, value) =>
        {
            var editor = (CodeEditor)view;
            if (!editor._synchronizingSelection)
                editor.TextView.SelectedRange = (RichTextRange)value;
        });
    ...
    public CodeEditor()
    {
        ...
        TextView.ContextMenuOpening += (_, args) =>
        {
            ...
            {
                args.Items.Clear();
                foreach (var item in menu.Cast<IMenuElement>())
                    args.Items.Add(item);
        ...
        TextView.Loaded += (_, _) =>
        {
            if (NativeAdapter is { } adapter)
                adapter.Post(() =>
                {
                    if (ReferenceEquals(NativeAdapter, adapter))
    ...
    private void SetDocument(CodeDocument? previous, CodeDocument document)
    {
        ...
        if (!ReferenceEquals(previous, document))
            DocumentChanged?.Invoke(this, new(previous, document));
    }
    ...
    private void OnContentChanged(object? sender, RichTextContentChangedEventArgs args)
    {
        NativeAdapter?.UpdateConfiguration();
        UpdateLines();
        if (args.ChangeSet.Origin == RichTextChangeOrigin.User)
            QueueNativeAutoIndent(args.ChangeSet);

        ContentChanged?.Invoke(this, args);
        if (args.ChangeSet.IsTextChanged)
            TextChanged?.Invoke(this, new RichTextTextChangedEventArgs(args.ChangeSet));
    }
    ...
    protected override void OnPropertyChanged([CallerMemberName] string? propertyName = null)
    {
        if (propertyName == nameof(BackgroundColor))
        {
            if (TextView is { } textView)
                textView.BackgroundColor = BackgroundColor;
            if (_gutter is { } gutter)
                gutter.BackgroundColor = BackgroundColor;
        }
    ...
    {
        if (ReferenceEquals(Lines.Text, Document.Text))
            return;
    ...
    private sealed class LineNumberDrawable(CodeEditor owner) : IDrawable
    {
        public void Draw(ICanvas canvas, RectF dirtyRect)
        {
            ...
            if (owner.NativeAdapter is not { } handler)
                return;
            if ((owner.TextColor ?? owner.TextView.TextColor ?? handler.GetTextColor()) is { } color)
